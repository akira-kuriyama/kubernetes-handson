...
* Secretでは一応エンコードされますが、値はbase64エンコードされているだけです。Kubernetesにアクセス可能なユーザーなら誰でも中身を見ることができますので、Secretという名前から安全と考えてはいけません。Secretが定義されたマニフェストファイルを暗号化する[kubesec](https://github.com/shyiko/kubesec)というものを使う選択肢があります。
